...
At every normal launch, select a recent project, open another project, or create a
project in any folder. `BlotNotebook` inside your Documents folder is offered as
the initial location. The ten most recent project paths are remembered. Use
**Switch project…** in the sidebar to change projects while the app is running.
Use `uv run app.py --project /path/to/project` to open one project directly for a
single run without changing the recent-project list. See [README](../README.md) for setup.

## Daily workflow

The workspace shows the current experiment above four workflow tabs, with the active Project's experiments on the left. **Normalization** keeps the classic clear layout: numerator/denominator and **Add calculation** at the top, direct **+ Group / Edit group / Remove group / Export CSV** controls, and the full per-lane **Ratio / Relative expression / Status** table always visible. A horizontal relative-expression table with lane selection below the results offers **Copy values** and **Copy with sample names**. **Raw Data** separates files and protein intensities into cards: use **Open** for archived/original locations and **Row options** for protein/source editing and pasted source history. **Remove file** remains directly visible. The workspace header also includes **Delete experiment**. Pages scroll when content needs more room.
...
12. **Delete experiment** removes the selected experiment and its archived copies from the Project after confirmation. Original files on disk are not touched.
...
The Project contains `project.json` and `experiments/<date_cell_condition_short-ID>/experiment.json`. Archived originals are stored directly below each experiment in `originals/<display filename>`. Attachment UUIDs remain in JSON and disambiguate same-name files with a short suffix. Experiment folder names remain stable after metadata edits. Older per-attachment UUID folders are flattened automatically. Original source files are not modified. Files are streamed to a SHA-256 hash after copying. The original absolute path and portable archive-relative path are both retained. **Open archived file** uses the operating system's associated application; TIFF rendering is outside this app.
...
To correct an accidental attachment, select it in **Raw Data** and click **Remove file**. Confirmation identifies the filename and linked protein rows. Removal detaches that attachment and its protein source links atomically; it does not change intensities or calculations. The original file is untouched. The archived bytes remain in the Project for traceability, and a `file_removed` entry in **History** retains the file metadata and archive path. This is removal from the experiment, not permanent disk deletion. Same-name attachments are distinguished by their IDs.

To back up or move to another computer, close the app and copy the entire Project folder. Open the copied Project on the other computer. The old original path may be unavailable there; the archived copy remains accessible. Use a Project on one computer at a time, on a local disk. This MVP is not a multi-user shared Project.

Older folders containing `wbquant.sqlite3` are imported into JSON when opened. The SQLite file is read-only during import and remains untouched as a backup. See [JSON Project storage](storage.md) for details.

## Architecture and schema

`app.py`: native PySide6 windows, dialogs, file drop, tables and clipboard workflow.

`core.py`: strict tab/CSV parser, calculations, portable archives, CSV export and atomic JSON persistence. UUIDs identify experiments, measurements, sources, calculations and groups. Lane identity is the number within an experiment. Experiment revisions detect stale edits from another window.
...
Tests cover core processing and an offscreen GUI startup with a temporary Project.
